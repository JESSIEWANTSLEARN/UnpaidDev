...
export default function PresenceTracker() {
  ...
  useEffect(() => {
    ...
    const sendActivity = async (force = false) => {
      ...
      if (
        !force &&
        now - lastServerSync.current < 10_000
      ) {
        return true;
      }
  ...
  const warningMs = policy
    ? policy.warningSeconds * 1000
    : null;

  const elapsedMs = policy
    ? Math.max(
        0,
        clock - lastHumanActivity.current,
      )
    : 0;

  const remainingMs = idleMs
    ? Math.max(0, idleMs - elapsedMs)
    : 0;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div>
          <p className="session-warning-label">
            Session Security
          </p>
          ...
          >
            No activity was detected. For your
